...
# creating classes for the spaceships, lasers etc
class Laser:
...
def main():
    ...
    # setting the fonts for the game
    ...
    # this is the function that updates the window at every loop
    def redraw_window():
        #draw background
        WIN.blit(BG, (0,0))
        
        # draw text for lives and levels
    ...
    while run:
        ...
        # 'update' enemis ships when they are all dead
        if len(enemies) == 0:
            level += 1
            wave_length += 5
        ...
        # map the movements of the ship and shooting
        ...
        if keys[pygame.K_s] and player.y + player_vel + player.get_height() +15 < HEIGHT: # moving down
            player.y += player_vel
        if keys[pygame.K_w] and player.y > player_vel: # moving up
            player.y -= player_vel
...
# define the initial menu before the game starts        
def main_menu():
    title_font = pygame.font.SysFont("comicsans", 70)
    
    run = True
    while run:
        # write the initial presse to begin the game
        WIN.blit(BG, (0,0))
